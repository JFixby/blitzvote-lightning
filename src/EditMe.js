...
import GrpcCaller from './grpc/GrpcCaller'
          ...
          {ANIMALS.map((animal, index) => {
            ...
            return (
              <Card path='/animals/' key={index} item={animal} select={this.selectAnimal} selected={selected} />
                ...
                <h3 className="select-territory">Vote for you candidate</h3>
                <div className="card-deck">
                    {TERRITORIES.map((territory, index) => {
                        const selected = myTerritory && myTerritory.id === territory.id
                        return (
                            <Card path='/territories/' key={index}
                                  item={territory} select={this.selectTerritory}
                                  selected={selected}/>
                        )
                    })
                    }
                ...
                <div className="container row">
                    <div className="col-lg-12 done">
                        <p>
                            <Link to={`/kingdom/${username}`}
                                  className="btn btn-primary"
                                  disabled={!completed}>Done</Link>
                        </p>
                    </div>
                </div>
            </div>
